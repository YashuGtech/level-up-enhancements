...
// ───────────────────────── Preview / Dev login ─────────────────────────
// Only used when the app is loaded on a Lovable preview host where the
// Telegram Login Widget refuses to render (bot domain not whitelisted).
// Creates / reuses a synthetic user so the rest of the app behaves normally.

const PREVIEW_TG_ID = -424242;

export const webLoginPreview = createServerFn({ method: "POST" }).handler(async () => {
  const { data: existing } = await db
    .from("users")
    .select("telegram_id")
    .eq("telegram_id", PREVIEW_TG_ID)
    .maybeSingle();

  if (!existing) {
    await db.from("users").insert({
      telegram_id: PREVIEW_TG_ID,
  ...
  const token = await issueSession(PREVIEW_TG_ID);
  return { token, telegramId: PREVIEW_TG_ID };
});
