...
function isPreviewHost(host: string) {
  // Lovable preview / staging hosts where Telegram widget can't render
  // (bot domain whitelisted only for the production custom domain).
  return (
...
function AuthPage() {
  ...
  // Mount Telegram Login Widget (production hosts only).
  useEffect(() => {
    if (preview) return;
  ...
  }, [preview, navigate, signInWithWebToken]);
      ...
      >
        <GoldFrame glow className="p-6 text-center">
          ...
          {countdown !== null ? (
            ...
            <div className="mt-6 space-y-3">
              <div className="rounded-md border border-gold-soft/30 bg-gold-soft/5 px-3 py-2 text-[11px] text-gold-soft">
                Preview mode detected. Telegram Login is only available on the
                production domain (gtcflappygame.com). Use the dev sign-in below
                to test the app here.
              </div>
              <button
                disabled={busy}
                onClick={() => void handlePreviewLogin()}
                className="w-full rounded-md bg-gradient-gold-flat px-4 py-3 text-sm font-semibold text-primary-foreground disabled:opacity-60"
              >
